...
static void emit_constant(compiler *compiler, value value) {
    emit2(compiler, OP_CONSTANT, add_constant(compiler, value));
}
...
static void compiler_bin_expr(compiler *compiler, expr_bin expr_bin) {
...
void compile_expr(compiler *compiler, expr *expr) {
    ...
    case EXPR_BIN: {
        compiler_bin_expr(compiler, expr->data.expr_bin);
        break;
    }
    default: {
        todo(compiler);
